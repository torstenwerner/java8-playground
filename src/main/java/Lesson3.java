...
public class Lesson3 {
    ...
    static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
        ...
        intStream.forEach(i -> computeDistances(wordList, i, distances[i]));

        return distances;
    }

    private static void computeDistances(List<String> wordList, int i, int[] distance) {
        final int LIST_SIZE = wordList.size();
        IntStream.range(i + 1, LIST_SIZE)
                .forEach(j -> { distance[j] = Levenshtein.lev(wordList.get(i), wordList.get(j)); });
    }
